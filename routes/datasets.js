...
const datasets = module.context.collection('datasets');
...
router.get(function (req, res) {
  res.send(datasets.all());
}, 'list')
.response([Dataset], 'A list of datasets.')
...
router.put(':key', function (req, res) {
  ...
  let meta;
  try {
    meta = datasets.replace(key, dataset);
  } catch (e) {
    if (e.isArangoError && e.errorNum === ARANGO_NOT_FOUND) {
      throw httpError(HTTP_NOT_FOUND, e.message);
    }
